...
export default function AdminManageOrderingFood() {
  ...
  const handleApprove = async (bookingId) => {
    const tableId = prompt('Enter table ID (e.g., t1, t2, t3...)');
    if (!tableId) return;

    setLoading(true);
    try {
      const response = await api.bookings.updateStatus(bookingId, 'CONFIRMED', tableId);
      setBookings(bookings.map(b => b.id === bookingId ? response.data : b));
      alert(response.message);
      // Refresh tables list
      const tablesRes = await api.tables.listAvailable();
      setTables(tablesRes.data);
    } catch (err) {
      alert('Error: ' + err.message);
    } finally {
